...
struct Farmacie* copiazaFarmaciiMici(struct Farmacie* farmacii, int nrFarmacii, float pragSuprafata, int* nrFarmaciiMici)
{
	*nrFarmaciiMici = 0;
	...
	for (int i = 0; i < nrFarmacii;i++)
	{
		if (farmacii[i].suprafata < pragSuprafata)
		{
			vector[i] = initializareFarmacie(farmacii[i].id, farmacii[i].nume, farmacii[i].suprafata);
		}
	}
	return vector;
}
...
int main() {
	//vector str de data omogena, liniara, elementele sunt asezate unul dupa altul si ocupa o zona de memorie contigua (Asta ne permite accesul direct)

	//ne cream un vector de farmacii
	int nrFarmacii = 4;
